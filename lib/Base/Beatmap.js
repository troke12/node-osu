 ...
 * @prop {String} hasVideo
 * @prop {String} hasDownload
 * @prop {String} hasAudio
...
class Beatmap {
	constructor(config, data) {
		...
		this.genre = num(data.genre_id)
		this.language = num(data.language_id);

		this.rating = num(data.rating);
		this.bpm = num(data.bpm);
		this.mode = num(data.mode);
		this.tags = data.tags;
		this.approvalStatus = num(data.approved);
		...
		this.hasVideo = parseBool(data.video);
		this.hasDownload = parseBool(data.download_unavailable);
		this.hasAudio = parseBool(data.audio_unavailable);
	}
	...
	get approvedDate() {
		...
		this._approvedDate = this.raw_approvedDate ? new Date(this.raw_approvedDate + ' UTC') : '0001-01-01T00:00:00Z';
		return this._approvedDate;
	}
